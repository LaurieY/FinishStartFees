...
{  public struct DateandValue
    ...
    class FeesSheet
    {
        ...
        public FeesSheet()
        {
            fileCols = new Dictionary<string, int>();
        ...
        public void getvariousCols(IWorksheet sheet)
        {
            ...
            fileCols["feeCol"] = resultAll[0].Column;
            fileCols["balanceCol"] = resultAll[1].Column;
            // the date column is the first text field of the row
            fileCols["txnDateCol"] = reciboRow.FindFirst("01/01", ExcelFindType.Text).Column;
            //The txnSeqCol is the next but 1 number after the date.
            //fileCols["txnSeqCol"] = findnumberaftertext(reciboRow, "01/01")[1];
            ...
            foreach (IRange aCell in reciboRow.Cells)
            {
                ...
                { if(((aCell.HasString)||(aCell.HasNumber)) &&(    int.TryParse(aCell.DisplayText.Trim(),out aNumber))) {
                    ...
                    if (numberNumber == 2)
                    {
                        fileCols["txnSeqCol"] = aCell.Column;
                        break;
                    }
            ...
            fileCols["propindexCol"] = sheet.FindFirst("4300", ExcelFindType.Text).Column;


            //fileCols["propnameCol"] = sheet.FindFirst("COBRO REC", ExcelFindType.Text).Column;// TODO: make a function, find numberaftertext
            fileCols["payCol"] = findnumberaftertext(sheet, "COBRO REC")[0];
            fileCols["propnameCol"] = sheet.FindFirst("PARC.", ExcelFindType.Text).Column;
            //Don't actually need to look for the balance cos already acquired it - 
            fileCols["sumaCol"] = sheet.FindFirst("Suma total", ExcelFindType.Text).Column;
        ...
        {// scan through the file acquiring propertID, propertyName, start balances and finish balances
         ...
         // set the sheet range to only include the PropertyID and search for then all
            ...
            int propIndexCol = fileCols["propindexCol"];
            ...
            {  //First get the endRows and and later go through again for starting balance, all fees and all payments and endbalance
                ...
                foundRow = sheet.Range[(int)fileScan[prop]["startRow"], fileCols["sumaCol"], lastRow, fileCols["balanceCol"]].FindFirst("Suma total", ExcelFindType.Text);//.EntireRow;
            ...
            {  //go through again for starting balance (if any) , all fees and all payments and endbalance
                ...
                foundRow = sheet.Range[(int)fileScan[prop]["startRow"], 1, (int)fileScan[prop]["endRow"], fileCols["balanceCol"]].FindFirst("ASIENTO DE APERTURA", ExcelFindType.Text);//.EntireRow;
                if ((foundRow != null))
                {
                    //  fileScan[prop]["startbalance"] =                        foundRow.Row;
                    fileScan[prop]["startBalance"] = (decimal) sheet[foundRow.Row, fileCols["balanceCol"]].Number;
                    //startRow depends on whether there is a ASIENTO DE APERTURA Row
                    // If no ASIENTA startRow is 1 more than the propert row
                    //if there is an ASSIENTA row then startRow is 1 more than that
                ...
                // foundRow = sheet.Range[(int)fileScan[prop]["startRow"], 1, (int)fileScan[prop]["endRow"], fileCols["balanceCol"]].FindFirst("Suma total", ExcelFindType.Text);//.EntireRow;
                fileScan[prop]["feeTotal"]  = sumofNumberCols(sheet, prop,  "feeCol");
                fileScan[prop]["payTotal"] = sumofNumberCols(sheet, prop, "payCol");
                ...
                fileScan[prop]["fees"] = detailNumberCols(sheet, prop, "feeCol");
                fileScan[prop]["payments"] = detailNumberCols(sheet, prop, "payCol");

                // foundRow = sheet[foundRow.Row, fileCols["balanceCol"]];//.Number;
       ...
       * the key of the dictionary is the transaction date
        ...
        public List<DateandValue> detailNumberCols(IWorksheet sheet, long property, string colName) {
           ...
           DateandValue nums = new DateandValue();
            ...
            IRange[] setofNumberCols = sheet.Range[(int)fileScan[property]["startRow"],
                 fileCols[colName], (int)fileScan[property]["endRow"] - 1, fileCols[colName]].Cells;
            int txnDateCol = (int)  fileCols["txnDateCol"]-1;   // cells is zero based
           // int txnSeqCol= (int)fileCols["txnSeqCol"] - 1;   // cells is zero based
        ...
        ***************/
        ...
        public decimal sumofNumberCols(IWorksheet sheet, long property, string colName) // TODO: also save individual entries for the simolify task leading to OCM exctract
        {
            IRange[]  setofNumberCols = sheet.Range[(int)fileScan[property]["startRow"], 
                fileCols[colName], (int)fileScan[property]["endRow"]-1, fileCols[colName]].Cells;
            decimal colTotal = 0;
